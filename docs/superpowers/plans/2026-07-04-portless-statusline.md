...
- [ ] **Step 1: statusline.py の既存差分を単独コミット**
...
- [ ] **Step 2: claude/CLAUDE.md の既存差分を単独コミット**
...
- [ ] **Step 2: [USER ACTION] CA 信頼と proxy 起動（sudo 対話が必要）**
...
- [ ] **Step 1: 失敗するテスト（フィクスチャで URL が出ないことを確認）**
...
- [ ] **Step 3: テスト再実行（起動中ルート → URL 表示）**

Step 1 と同じコマンドを実行。

Expected: Line 2 に `🌐 https://test-app.localhost` が含まれる

- [ ] **Step 4: 死んだ pid → 非表示 を確認**
...
- [ ] **Step 5: worktree prefix（feature/foo → foo.）を確認**
...
- [ ] **Step 6: portless 不在環境フォールバック（state dir なし）を確認**
...
- [ ] **Step 2: symlink 経由で反映されていることを確認**
...
- [ ] **Step 2: portless run で起動（バックグラウンド）**
...
- [ ] **Step 3: statusline が実 state で URL を出すことを確認**
...
- [ ] **Step 4: worktree で prefix 付き URL を確認**
